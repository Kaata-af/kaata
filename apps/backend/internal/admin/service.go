...
// KV is a generic {key, count} breakdown row.
type KV struct {
	Key   string `json:"key"`
	Count int64  `json:"count"`
}

// WaitlistRow is one signup (admin-only, so the email is shown). `platform` is
// the store the visitor asked about; `device` is what they browsed on.
...
type WaitlistResult struct {
	...
	ByPlatform  []KV          `json:"by_platform"`
	ByDevice    []KV          `json:"by_device"`
	Recent      []WaitlistRow `json:"recent"`
	GeneratedAt string        `json:"generated_at"`
}

// GetWaitlist returns the waitlist signups for the operator dashboard: a total,
// breakdowns by requested store and by detected device, and the most recent
// rows (emails included — this is behind AdminKeyMiddleware).
func (s *Service) GetWaitlist(ctx context.Context) (WaitlistResult, error) {
	res := WaitlistResult{ByPlatform: []KV{}, ByDevice: []KV{}, Recent: []WaitlistRow{}}

	if err := s.pool.QueryRow(ctx, `SELECT COUNT(*) FROM waitlist`).Scan(&res.Total); err != nil {
		return res, err
	}

	loadKV := func(query string) ([]KV, error) {
		out := []KV{}
	...
	var err error
	if res.ByPlatform, err = loadKV(
		`SELECT platform, COUNT(*) FROM waitlist GROUP BY platform ORDER BY COUNT(*) DESC, platform`,
	); err != nil {
		return res, err
	}
	if res.ByDevice, err = loadKV(
		`SELECT COALESCE(device, 'unknown'), COUNT(*) FROM waitlist
	...
	rows, err := s.pool.Query(ctx, `
		SELECT email, platform, COALESCE(device, 'unknown'),
	...
	for rows.Next() {
		var r WaitlistRow
		if err := rows.Scan(&r.Email, &r.Platform, &r.Device, &r.CreatedAt); err != nil {
			return res, err
		}
		res.Recent = append(res.Recent, r)
	}
	if err := rows.Err(); err != nil {
		return res, err
	}
