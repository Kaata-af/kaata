...
// --- Ed25519 ---
const edSeed = fill(32, 0);
for (let i = 0; i < 32; i++) edSeed[i] = i;
...
function computeContiguous(seqs) {
  ...
  for (; i < sorted.length; i++) {
    const s = sorted[i];
    if (s > prev + 1) gaps.push({ from_seq: prev + 1, to_seq: s - 1 });
    prev = s;
  }
...
const hlc = {
  recv_same_ms: tickReceive(
    { pms: 100, l: 2, did: "d1" },
    { pms: 100, l: 5, did: "d2" },
    100,
    "d1",
  ),
  recv_remote_ahead: tickReceive(
    { pms: 100, l: 2, did: "d1" },
    { pms: 200, l: 0, did: "d2" },
    150,
    "d1",
  ),
  recv_drift_clamp: tickReceive(
    { pms: 100, l: 0, did: "d1" },
    { pms: 1000000, l: 0, did: "d2" },
    100,
    "d1",
  ),
...
const pop = {
  ...
  transcriptHex: hex(
    buildPopMessageV3(["evt-b", "evt-a", "evt-c"], "nonce123", fill(32, 0x33), fill(32, 0x44)),
  ),
};
...
function deriveRfcommUuid(secret) {
  ...
  const hx = Array.from(h.slice(0, 16))
  ...
  JSON.stringify(
    {
      ...
      canonicalizeEvent: {
        canonical: canonStr,
        canonicalHex: hex(canonBytes),
        eventSigB64Std: b64(evtSig),
        signerPubB64Std: b64(edPub),
      },
    },
    null,
    2,
