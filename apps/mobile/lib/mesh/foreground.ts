...
/**
 * Register the foreground-service task BEFORE the service ever starts.
 * Notifee requires this so it has a JS handler to keep alive while the
 * notification is showing. The task just awaits a never-resolving promise —
 * we tear down via stopShopModeForegroundService().
 *
 * Call from _layout.tsx top level, NOT from inside a component.
 */
export function registerShopModeForegroundTask(): void {
  const notifee = getNotifee();
  if (!notifee) return;
  try {
    notifee.default.registerForegroundService(
      () =>
        new Promise<void>(() => {
          // Intentionally never resolves. Notifee kills the task when
          // the notification with asForegroundService: true is cancelled.
        }),
    );
  } catch (err) {
    if (__DEV__) console.warn("[mesh/foreground] registerShopModeForegroundTask failed", err);
  }
}
  ...
  try {
    await notifee.default.displayNotification({
      ...
      android: {
        ...
        // D-FOREGROUND-CRASH: REQUIRED for the foreground-service
        // Notification to build. Without a valid drawable, Service
        // .startForeground() is never called and Android force-kills us
        // after 5s with ForegroundServiceDidNotStartInTimeException.
        // ic_launcher is always present in an Expo Android build.
...
export async function updateShopModeNotification(opts: ShopModeNotificationOpts): Promise<void> {
  ...
  try {
    await notifee.default.displayNotification({
      ...
      android: {
        channelId: SHOP_MODE_CHANNEL_ID,
        // D-FOREGROUND-CRASH: same smallIcon contract as start — the
