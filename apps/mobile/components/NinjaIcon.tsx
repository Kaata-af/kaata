...
// Ninja-head icon for the onboarding "Stay fully offline" card.
//
// Single-path silhouette: a circular ninja face with a horizontal
// headband across the eyes, the band's trailing ties on either side,
// and a small katana-style ornament breaking out of the top-right.
// Eyes and headband fabric show through as cutouts via the SVG path's
// nonzero winding rule (the inner subpaths wind opposite to the head
// silhouette, creating "holes" filled with the card background).
//
// viewBox "0 -64 640 640" inherits from the source artwork — the
// vertical offset accommodates the katana hilt sticking above the
// head circle.

type NinjaIconProps = {
  size?: number;
  // Solid fill color for the head silhouette + tied ends. Eyes and
  // headband fabric are cutouts so they read as the card background.
  color?: string;
};

export function NinjaIcon({ size = 32, color = colors.textEmphasis }: NinjaIconProps) {
  return (
    <Svg width={size} height={size} viewBox="0 -64 640 640" fill="none">
      <Path
