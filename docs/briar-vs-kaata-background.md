...
> **Briar's sync engine *is* the background service — plain JVM threads living in
...
1. **The notification vanishes on swipe** because our foreground service is a
   *faithful but incomplete* port of Briar's — it's missing the tricks that keep
...
| **OEM wake-lock-tag spoofing** | The lock's *tag* is disguised as a system service the OEM killer whitelists: `"LocationManagerService"` if Huawei PowerGenie is installed, `"AudioIn"` for Evenwell/Asus, else the package name. It literally masquerades as a system service. | `AndroidWakeLockManagerImpl.getWakeLockTag()` |
...
| **Guided OEM settings** | In-app flows to the Doze battery-optimization whitelist *and* the Xiaomi/MIUI Security-Center "Lock apps"/autostart screens (version-detected). Briar concedes code alone can't win on MIUI — it coaches the user. | `dont-kill-me-lib/.../DozeUtils.kt`, `XiaomiUtils.kt` |
| **Return code** | `START_NOT_STICKY` — Briar *doesn't* rely on auto-restart. Liveness comes from the FGS+wakelock+alarm making the process expensive to kill and quick to wake, not from respawning. | `BriarService.java` |

The load-bearing insight: **all of Briar's reliability tricks protect a process
whose worker is already inside it.** The wake lock keeps the *sync threads*
warm. There's nothing to "revive."

## What kaata does (from our source)

We ported the *shape* of Briar's service but the engine lives elsewhere:
...
| **Alarm backstop** | **None.** The 90s tick is an in-process `Handler` that dies with the process; only `START_STICKY` brings the FGS back, and on Android 12+ restarting an FGS from the background is *blocked* (`if (!entered) stopSelf()`), so a real process kill = silence until reopen. | `KaataForegroundService.kt:198-208` |
...
3. **No alarm backstop.** Once the process *is* gone, Briar's `setAndAllowWhileIdle`
...
*direction* — our residency stack is strictly weaker than Briar's — is certain
from the source.)
...
- On **MIUI/Huawei**, app-killing is undocumented OEM policy with "no APIs and no
  ...
  guided flows for exactly this and *still* tells MIUI users to flip those
  switches.
...
- As long as our sync is in **JS**, we're one process-death away from total
  silence until reopen — the headless path is bounded and best-effort *by
  construction* (RN: "spin up, run the task, spin down").
...
and *still* leans on user settings. Before pouring weeks into (B), weigh it
...
not. Background BT sync is the right tool for *multi-device one-shop* offline use;
the server is the right tool for *two-party always-agree*. They're different jobs.
