...
- Update `CLAUDE.md` + `docs/architecture.md`: the "customer ledger never leaves the device" claim is stale — signed-in sync uploads names/phones/amounts (H7). Now also: currency follows the active vault (H19), recovery is non-destructive on populated vaults (B9). ~~self-identity check-in is gated on sign-in (B5)~~ — **B5 reversed 2026-07-10** (operator decision): self identity is sent on check-in regardless of sign-in, needed for offline-user analytics/outreach; the privacy policy + `docs/play-data-safety.md` already disclose exactly this collection.
...
- `app.json` (H3) — **partially done.** Removed `FOREGROUND_SERVICE_CONNECTED_DEVICE` + `REQUEST_IGNORE_BATTERY_OPTIMIZATIONS` from permissions and dropped `./plugins/withKaataForegroundService` (the connectedDevice FGS declaration), so the Play foreground-service declaration no longer applies and the battery-optimization policy risk is gone. **Still injected by the compiled-in native modules** and NOT yet stripped: the Bluetooth permissions (`BLUETOOTH_SCAN/CONNECT/ADVERTISE/BLUETOOTH/BLUETOOTH_ADMIN`, from `react-native-ble-plx` + `react-native-ble-advertiser`) and `CAMERA` (`expo-camera`, used only by the hidden pair-scan). Fully clearing those needs either the mesh un-park or removing those native deps + their screens (a build-verified change) — they're install-time / Data-safety friction, not a submission blocker. Also still present: `NEARBY_WIFI_DEVICES` / `CHANGE_WIFI_MULTICAST_STATE` / `ACCESS_WIFI_STATE` / `WAKE_LOCK` (entangled with `withNearbyWifiNeverForLocation`). Verify the shipped AAB with `aapt dump permissions` / the Play permission list.
