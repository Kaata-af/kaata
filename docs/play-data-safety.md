...
### Financial info
- Other financial info — **✅ Yes** (the ledger debt/credit **amounts** — Google's definition of "Other financial info" literally names *"debts"*)
...
| App info & performance → Crash logs | App functionality (fix bugs) | **Required** (automatic) |
...
- **Is all user data encrypted in transit?** → **Yes.** All requests go to `https://api.kaata.af` (TLS). *(Note for your own awareness, not a form field: the ledger is sent as plaintext JSON over TLS — it is not additionally end-to-end encrypted.)*
...
- Crash-report `message` strings are truncated and *intended* to exclude PII, but aren't guaranteed 100% free of user-typed text — covered by declaring **Crash logs**.
